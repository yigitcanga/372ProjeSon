...
ALTER TABLE kurumgiderleri 
ADD COLUMN tarih date;

select *from KurumGiderleri;
TRUNCATE TABLE kurumgiderleri ;
...
SELECT TIMESTAMPDIFF(WEEK, k.tarih, '2023-08-07') AS week_difference
FROM KurumGiderleri k
where k.gider_id = 5003;

select k.tutar from kurumgiderleri k; -- haftalık gider tutarı

select k.tutar*4 from kurumgiderleri k; -- aylık 
  
 
-- SELECT PERIOD_DIFF(DATE_FORMAT('2023-08-07', '%Y%m'), DATE_FORMAT('2023-04-03', '%Y%m')) AS ay_sayisi;
-- SELECT TIMESTAMPDIFF(WEEK, '2023-04-03','2023-08-07') AS hafta_sayisi;
...
delete from Öğrenci where okul_no  = 3;
select *from veli;
select *from Öğrenci where okul_no = 1;
insert into Öğrenci values(1,'Elif','Yılmaz','1998-04-15','elifyilmaz@gmail.com',5931892251,3,'İstanbul/Beyoğlu/Galata',1,6530,80375008223);
select *from Öğrenci;
select *from veli;
delete from personel  where personel_id  = 682;
select *from personel t;
select *from parttimeçalışan p;

select Öğrenci.ad,Öğrenci.soyad from Öğrenci;
select *from Personel where personel_id = 501;
SELECT
  ders.ders_adı,
  ad.kontenjan,
  ad.talep,
...
WHERE
 ...
 select *from kurumgiderleri where kurumgiderleri.gider_id = 5000 ;
  
select *from stok where stok.stok_id = 3000;
